...
int DoRpcCall(const std::shared_ptr<stu::UniversityServiceProxy> &proxy) {
  ::trpc::ClientContextPtr client_ctx = ::trpc::MakeClientContext(proxy);
  stu::PingMessage req;
  req.set_ping("ping!");
  stu::PingMessage rsp;
  ::trpc::Status status = proxy->Ping(client_ctx, req, &rsp);
  if (!status.OK()) {
    std::cerr << "get rpc error: " << status.ErrorMessage() << std::endl;
    return -1;
  }
  std::cout << "get rsp msg: " << rsp.ping() << std::endl;
  return 0;
}

int Run() {
  const auto proxy =
      ::trpc::GetTrpcClient()->GetProxy<stu::UniversityServiceProxy>(
          FLAGS_service_name);
